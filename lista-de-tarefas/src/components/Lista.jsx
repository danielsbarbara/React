export const Lista = ({ lista, deleteItem, changeConcluido, checkConcluido }) => {
    ...
    return (
        <div> {
            lista.map((ele,id) => <Tela key={id} conteudo={ele} deleteItem={deleteItem} checkConcluido={checkConcluido} lista={lista} className='Tela'/>)
        }
        </div>
    )
}
...
const Tela = ({ conteudo, deleteItem, checkConcluido}) => {
    ...
    return (
        <div className="formulario">
            ...
            <h3 className={conteudo.concluido ? 'check' : 'Tela'}>{conteudo.descricao}</h3>
